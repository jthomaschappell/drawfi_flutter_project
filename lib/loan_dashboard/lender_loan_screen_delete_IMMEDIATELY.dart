...
class LenderLoanScreenDeleteImmediately extends StatefulWidget {
  final String loanId;

  const LenderLoanScreenDeleteImmediately({super.key, required this.loanId});

  @override
  State<LenderLoanScreenDeleteImmediately> createState() => _LenderLoanScreenDeleteImmediatelyState();
}

class _LenderLoanScreenDeleteImmediatelyState extends State<LenderLoanScreenDeleteImmediately> {
  ...
  Future<bool> _checkFileExists(String filePath) async {
    try {
      developer.log('Starting file check',
          name: 'Storage', error: {'filePath': filePath});
      ...
      try {
        ...
        developer.log('File found with URL',
            name: 'Storage', error: {'filePath': filePath, 'url': url});

        return true;
      } catch (e) {
        developer.log('File access failed, checking buckets',
            name: 'Storage',
            error: {'error': e.toString(), 'filePath': filePath});
      }

      // If direct access fails, list the bucket contents

      final List<FileObject> files =
          await supabase.storage.from('project_documents').list();

      developer.log('Found files in bucket',
          name: 'Storage', error: {'fileCount': files.length});

      // Check if file exists in root or any subfolder

      for (var file in files) {
        if (file.name == filePath || filePath.endsWith(file.name)) {
          developer.log('File found in bucket',
              name: 'Storage',
              error: {'filePath': filePath, 'location': 'root'});

          return true;
        }
      }

      developer.log('File not found in bucket',
          name: 'Storage', error: {'filePath': filePath});

      return false;
    } catch (e) {
      developer.log('File check failed',
          name: 'Storage',
          error: {'error': e.toString(), 'filePath': filePath});
  ...
  Future<void> _downloadFile(String fileUrl, String fileName) async {
    try {
      ...
      developer.log('Download initiated',
          name: 'FileDownload',
          error: {'fileUrl': fileUrl, 'fileName': fileName});
      ...
      developer.log('Signed URL generated',
          name: 'FileDownload', error: {'signedUrl': signedUrl});
      ...
      developer.log('Download triggered',
          name: 'FileDownload', error: {'fileName': fileName});
    } catch (e, stackTrace) {
      // Log error details
...
// Helper method to handle file opening
  ...
  Future<void> _openFile(String fileUrl, String fileName) async {
    try {
      developer.log('Opening file initiated',
          name: 'FileOpen', error: {'fileUrl': fileUrl, 'fileName': fileName});
      ...
      developer.log('File open error',
          name: 'FileOpen',
          error: {'error': e.toString(), 'fileName': fileName});
    ...
    try {
      ...
      for (var ref in fileRefs) {
        ...
        if (!fileExists) {
          ...
          developer.log('Removed orphaned file reference',
              name: 'Cleanup', error: {'fileName': ref['file_name']});
        }
      }
    } catch (e) {
      developer.log('Cleanup error',
          name: 'Cleanup', error: {'error': e.toString()});
    }
  }
...
Widget _buildStatusBadge(String status) {
  ...
  Future<void> _downloadMultipleFiles(List<Map<String, dynamic>> files) async {
    try {
      ...
      developer.log('Batch download error',
          name: 'FileDownload',
          error: {'error': e.toString(), 'filesCount': files.length});
...
>>>>>>> 53cf4c6fe3eca97e7fe71296186f58b1090fbf1d
      ),
    );
  }
