...
class NotificationService {
  ...
  Stream<List<ConstructionNotification>> subscribeToUpdates() {
    final loansStream = supabase.from('construction_loans').stream(
      primaryKey: ['loan_id'],
    ).map(
      (changes) => _processLoanChanges(
        changes,
      ),
    );
    ...
    final lineItemsStream = supabase
        ...
        .stream(primaryKey: ['category_id']).map(
            (changes) => _processLineItemChanges(changes));
  ...
  List<ConstructionNotification> _processLoanChanges(
  ...
  List<ConstructionNotification> _processLineItemChanges(
      List<Map<String, dynamic>> changes) {
    return changes.map((change) {
      return ConstructionNotification(
        id: const Uuid().v4(),
